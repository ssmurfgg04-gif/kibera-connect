...
mkdir -p "$W"
rm -f "$W"/*.mp4 "$W"/list.txt

FPS=30
HEAD=0.35   # silence before narration (s)
TAIL=0.75   # silence after narration (s)

# segments: "image|audio|zoomdir"  zoomdir: in|out|none-silent-duration
build_seg () {
  ...
  if [ "$aud" = "none" ]; then
    ...
    ffmpeg -y -loop 1 -framerate $FPS -i "$img" -f lavfi -i anullsrc=r=48000:cl=stereo \
      ...
      -map "[v]" -map "[a]" -t "$dur" -c:v libx264 -preset medium -crf 18 -c:a aac -b:a 192k -ar 48000 "$out" -loglevel error
    echo "seg$idx (silent ${dur}s) -> $out"
    return
  fi

  dur=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "$aud")
  total=$(python3 -c "print(round($dur + $HEAD + $TAIL, 2))")
  ...
  ffmpeg -y -loop 1 -framerate $FPS -i "$img" -i "$aud" \
    ...
    -map "[v]" -map "[a]" -t "$total" -c:v libx264 -preset medium -crf 18 -c:a aac -b:a 192k -ar 48000 "$out" -loglevel error
  echo "seg$idx (${total}s) -> $out"
}

# 1. Title card (silent opening)
...
# Concat
: > "$W/list.txt"
for f in "$W"/seg*.mp4; do echo "file '$f'" >> "$W/list.txt"; done
ffmpeg -y -f concat -safe 0 -i "$W/list.txt" -c copy "$W/concat.mp4" -loglevel error

# Global fades + loudness normalize
...
ffmpeg -y -i "$W/concat.mp4" \
  ...
  "/home/z/my-project/download/kiberaconnect-demo.mp4" -loglevel error

echo "FINAL: /home/z/my-project/download/kiberaconnect-demo.mp4 (${TOTAL}s)"
