/**
 * The seed, as source of truth. Shared between the build-time seeder
 * (scripts/seed.ts) and the runtime bootstrap in lib/db.ts, so a lambda
 * that boots without a bundled database can rebuild the same Kibera.
 */
...
export const VILLAGES: Record<string, { lat: number; lng: number }> = {
...
export function jitter(v: { lat: number; lng: number }, seed: number) {
